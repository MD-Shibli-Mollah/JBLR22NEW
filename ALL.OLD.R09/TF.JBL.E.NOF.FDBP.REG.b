SUBROUTINE TF.JBL.E.NOF.FDBP.REG(Y.RETURN)
*-----------------------------------------------------------------------------
*
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
    ...
    !     Y.TO.DATE =  ENQ.SELECTION<4,TO.POS>
    ! END

    ! LOCATE 'CUS.NO' IN ENQ.SELECTION<2,1> SETTING CUS.POS THEN
    ! Y.CUS.NO = ENQ.SELECTION<4,CUS.POS>
    ! END
    ...
    ! Y.USER.CATEGORY = '3831'
    ! Y.CO.CODE = 'BD0010101'
    
    Y.CO.CODE =''
    Y.CO.CODE= ID.COMPANY
    Y.PRODUCT.GROUP = 'JBL.FDBP.LN'
    Y.PROP.CLASS =''
    Y.PROPERTY =''
    IF (Y.FROM.JUL EQ '' AND Y.TO.JUL EQ '') THEN
        SEL.CMD= " SELECT " :FN.AA.ARRANGEMENT : " WITH CO.CODE EQ " : Y.CO.CODE: " AND PRODUCT.GROUP EQ " : Y.PRODUCT.GROUP
    END
    ELSE IF ( Y.FROM.JUL NE '' AND Y.TO.JUL NE '') THEN
        SEL.CMD= " SELECT " :FN.AA.ARRANGEMENT : " WITH CO.CODE EQ " : Y.CO.CODE: " AND PRODUCT.GROUP EQ " : Y.PRODUCT.GROUP :" AND  START.DATE GE " : Y.FROM.JUL : " AND  START.DATE LE " : Y.TO.JUL
    END
