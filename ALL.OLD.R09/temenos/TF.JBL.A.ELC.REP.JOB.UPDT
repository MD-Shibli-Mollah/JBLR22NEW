SUBROUTINE TF.JBL.A.ELC.REP.JOB.UPDT
*-----------------------------------------------------------------------------
* This routine is used to update Sales Contract and JOB Register Information
* during ELC Replacement record authorization
*-----------------------------------------------------------------------------
* Description: this Routine is created by coping the full routine TF.JBL.A.BTB.ELC.REP expect the modification
* Modification History : 12/10/2020
* Modification Description: The previous routine(TF.JBL.A.BTB.ELC.REP) only update job register BTB Entitle Amt & PC Entitle Amt but
*                           bank demand Export Lc Replacement Update the job register as like Export LC Recording.
* Modifided Line :
* Line 48: POS(8 to 11) Added 4 local filed.
* Line 238 - 241
* Line 161 - 173
* Line 238 - 241
* Line 208 - 216
...
* update Replace ELC Ref No, Replacement Date and Covered Amt to Sales Contract
    ...
    R.SCT.REC<SCT.REP.ELC.DATE,Y.REP.ELC.REFNO.CNT> = EB.SystemTables.getToday()
    R.SCT.REC<SCT.ELC.COVERED.AMT,Y.REP.ELC.REFNO.CNT> = Y.TF.CONT.RAMT

*******************ADDED BY EBRAHIM************21 DEC 2020************
    R.SCT.REC<SCT.TOT.ELC.COVERED.AMT> += Y.TF.CONT.RAMT
...
**********************************UPDATE JOB EX REF*************************************
        ...
        Y.NEW.EX.AMT = Y.LC.REF<1,Y.EXCESS.REP.AMT.POS>
        Y.NEW.FOB.AMT = Y.LC.REF<1,Y.NET.FOBVL.POS>
        ...
        R.JOB.REC<BTB.JOB.EX.BTB.ENT.PER,Y.ELC.TF.REF.CNT> = Y.LC.REF<1,Y.BTB.RT.POS>
        R.JOB.REC<BTB.JOB.EX.PC.ENT.PER,Y.ELC.TF.REF.CNT> = Y.LC.REF<1,Y.PC.RT.POS>
***********************************************************************
...
*****************************************************
*    WRITE.FILE.VAR = "Y.EXCESS.REP.AMT.POS: ": Y.EXCESS.REP.AMT.POS:VM:" Y.NET.FOBVL.POS: ": Y.NET.FOBVL.POS:VM:" Y.BTB.RT.POS: ": Y.BTB.RT.POS:VM:" Y.PC.RT.POS: ": Y.PC.RT.POS
*    GOSUB FILE.WRITE
*    WRITE.FILE.VAR = "Y.TF.EXCHG.RTE ": Y.TF.EXCHG.RTE
*    GOSUB FILE.WRITE
*****************************************************
...
**********************************UPDATE JOB TOT EX & FOB AMT*************************************
        Y.EX.AMT = Y.NEW.EX.AMT * Y.TF.EXCHG.RTE
        Y.FOB.AMT = Y.NEW.FOB.AMT * Y.TF.EXCHG.RTE
        Y.TOT.EX.AMT =  R.JOB.REC<BTB.JOB.TOT.EX.LC.AMT>
        Y.TOT.FOB.AMT = R.JOB.REC<BTB.JOB.TOT.NET.FOB.VALUE>
    
        R.JOB.REC<BTB.JOB.TOT.EX.LC.AMT> = Y.TOT.EX.AMT + Y.EX.AMT
        R.JOB.REC<BTB.JOB.TOT.NET.FOB.VALUE> = Y.TOT.FOB.AMT + Y.FOB.AMT
*****************************************************
