...
* <Rating>0</Rating>
*-----------------------------------------------------------------------------
SUBROUTINE TF.JBL.E.CNV.SCDUL.TO.ISSBK1(IN.FREQ,HEADER.REC,MV.NO,OUT.FREQ,ERROR.MSG)
...
*F******************************
    ...
    IF Y.PRSNTOR.ID NE '' THEN
        ...
        IF REC.CUS THEN
            CUS1 = REC.CUS<ST.Customer.Customer.EbCusShortName>
            OUT.MASK = CUS1
        END
    END
    ELSE
