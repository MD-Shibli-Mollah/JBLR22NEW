...
SUBROUTINE CR.JBL.V.ARR.EXP.LN
*-----------------------------------------------------------------------------
*Subroutine Description: Validation routine for Expired Loan(AA or Acc Num)
*Subroutine Type:
*Attached To    :  Version - AA.ARRANGEMENT.ACTIVITY,JBL.AA.EXP.NEW
...
PROCESS:
    ...
    IF Y.ID.LEN EQ 'AA' THEN
        ...
        IF Y.PRODUCT.LINE EQ "ACCOUNTS" THEN
            Y.ACTIVITY = "ACCOUNTS-JBL.EXP.RESCHEDULE-ARRANGEMENT"
        END
        
        IF Y.PRODUCT.LINE EQ "LENDING" THEN
            Y.ACTIVITY = "LENDING-JBL.EXP.RESCHEDULE-ARRANGEMENT"
        END
    END
