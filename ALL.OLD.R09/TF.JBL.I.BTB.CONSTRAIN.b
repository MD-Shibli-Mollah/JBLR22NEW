...
SUBROUTINE TF.JBL.I.BTB.CONSTRAIN
*-----------------------------------------------------------------------------
* Sales Contract amend's data calculation and validation
*-----------------------------------------------------------------------------
* Modification History : Abu Hurair
* Attachted to: BD.SCT.CAPTURE,CONT.AMEND
*-----------------------------------------------------------------------------
    $INSERT I_COMMON
    $INSERT I_EQUATE
    $INSERT I_GTS.COMMON
...
*       Y.CONV.BTB.ENT.AMT = ABS((Y.BTB.ENT.AMT - Y.BTB.ENT.AMT.OLD) * Y.EXCHANGE.RATE) ;*to find BTB Entitlement changes Amount with Exchange rate
        Y.BTB.ENT.CHANGE.AMT = Y.BTB.ENT.AMT - Y.BTB.ENT.AMT.OLD ;*to find BTB Entitlement changes Amount with Exchange rate
        Y.BTB.ENT.OUST.AMT = Y.TOT.BTB.AVL.AMT + Y.BTB.ENT.CHANGE.AMT
*       IF Y.TOT.BTB.AVL.AMT LT Y.CONV.BTB.ENT.AMT  THEN ;* Check Reduced BTB Entitlement Amount is Available in BTB JOB Register
        IF Y.BTB.ENT.OUST.AMT LT '0'  THEN ;* Check Reduced BTB Entitlement Amount is Available in BTB JOB Register
...
*       Y.CONV.PCECC.ENT.AMT = ABS((Y.PCECC.ENT.AMT - Y.PCECC.ENT.AMT.OLD) * Y.EXCHANGE.RATE) ;*to find PC/ECC Entitlement changes Amount with Exchange rate
        Y.PCECC.ENT.CHANGE.AMT = Y.PCECC.ENT.AMT - Y.PCECC.ENT.AMT.OLD ;*to find PC/ECC Entitlement changes Amount with Exchange rate
        Y.PCECC.ENT.OUST.AMT = Y.TOT.PC.AVL.AMT + Y.PCECC.ENT.CHANGE.AMT
*       IF Y.TOT.PC.AVL.AMT LT Y.CONV.PCECC.ENT.AMT THEN ;* Check Reduced PC/ECC Entitlement Amount is Available in BTB JOB Register
        ...
        IF Y.PCECC.ENT.OUST.AMT LT '0' THEN ;* Check Reduced PC/ECC Entitlement Amount is Available in BTB JOB Register
            Y.OVERRIDE.VAL = EB.SystemTables.getRNew(EB.SystemTables.getV()-9)
            Y.OVRRD.NO = DCOUNT(Y.OVERRIDE.VAL,VM) + 1
